...
export default function GamificationCasesModal({ onClose }: GamificationCasesModalProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-[70] bg-black/90 p-2 backdrop-blur-md md:p-5"
      onClick={onClose}
      role="presentation"
    >
      <section
        ...
        className="mx-auto flex h-[calc(100dvh-1rem)] w-full max-w-[94rem] flex-col overflow-hidden bg-white text-[#111] md:h-[calc(100dvh-2.5rem)]"
      >
        <header className="relative shrink-0 border-b border-black/15 bg-white px-4 py-4 pr-16 md:px-7 md:py-5">
          <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#FD4B32]">
            Кейсы / Геймификация
          </p>
          <nav className="mt-3 flex gap-2 overflow-x-auto" aria-label="Кейсы геймификации">
          ...
          <button
            type="button"
            onClick={onClose}
            className="absolute right-4 top-4 flex h-10 w-10 cursor-pointer items-center justify-center border border-black/15 text-2xl font-light leading-none transition-colors hover:border-black hover:bg-black hover:text-white md:right-7 md:top-5"
        ...
        <div className="grid flex-1 overflow-y-auto lg:grid-cols-12">
          <div className="grid content-start gap-px bg-black/15 lg:col-span-7 lg:grid-cols-2">
            {activeCase.imageSlots.map((slot, index) => (
              <div
                key={`${activeCase.client}-${slot}`}
                className={`relative flex min-h-[18rem] items-center justify-center overflow-hidden bg-[#efefef] p-6 md:min-h-[25rem] ${
                  index === 0 ? "lg:col-span-2 lg:min-h-[32rem]" : ""
                }`}
              >
                <span className="absolute left-5 top-5 font-mono text-[10px] uppercase tracking-[0.18em] text-black/45">
                  {String(index + 1).padStart(2, "0")} / {String(activeCase.imageSlots.length).padStart(2, "0")}
                </span>
                <div className="text-center">
                  <span className="mx-auto mb-5 block h-10 w-10 bg-[#FD4B32]" />
                  <p className="font-headline text-xl font-semibold text-black/35">
                    {formatTypography(slot)}
                  </p>
          ...
          <div className="bg-white px-5 py-7 md:px-10 md:py-10 lg:col-span-5 lg:px-[clamp(2rem,3.5vw,4rem)]">
            <p className="font-mono text-xs font-bold uppercase tracking-[0.16em] text-[#FD4B32]">
              {activeCase.client}
            </p>
            <h2
              id="gamification-case-title"
              className="mt-5 max-w-[12ch] font-headline text-[clamp(2.3rem,4vw,5rem)] font-semibold leading-[0.9] tracking-[-0.045em]"
            >
              {formatTypography(activeCase.title)}
            </h2>
            <p className="mt-5 max-w-xl font-sans text-[clamp(1.05rem,1.35vw,1.35rem)] leading-tight text-black/60">
            ...
            <div className="mt-12">
              ...
              <div className="mt-5 space-y-5">
                {activeCase.paragraphs.map((paragraph) => (
                  <p key={paragraph} className="font-sans text-base leading-[1.3] text-black/80 md:text-lg">
            ...
            <div className="mt-12 pb-8">
              ...
              <div className="mt-5 border-t border-[#FD4B32]/45">
                {activeCase.metrics.map((metric) => (
                  <div
                    key={`${metric.value}-${metric.label}`}
                    className="grid grid-cols-[minmax(7rem,auto)_1fr] items-end gap-5 border-b border-[#FD4B32]/45 py-5"
                  >
                    <strong className="font-headline text-[clamp(2.6rem,4vw,5rem)] font-semibold leading-none tracking-[-0.04em] text-[#FD4B32]">
                      {metric.value}
                    </strong>
                    <span className="pb-1 text-right font-sans text-sm leading-tight text-[#FD4B32] md:text-base">
